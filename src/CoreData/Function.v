...
Definition compose12 {A B C D}
    : (C -> D) -> (A -> B -> C) -> A -> B -> D :=
  compose compose compose.
Arguments compose12 {A B C D} _ _ _ _ /.
Definition compose13 {A B C D E}
    : (D -> E) -> (A -> B -> C -> D) -> A -> B -> C -> E :=
  compose compose12 compose.
Arguments compose13 {A B C D E} _ _ _ _ _ /.

Definition const {A B} (a:A) (_:B) : A := a.
Arguments const {A B} a _ /.
Definition const2 {A B C} : A -> B -> C -> A := compose const const.
Arguments const2 {A B C} _ _ _ /.
...
Module FunctionNotation.
  ...
  Infix "'.:'" := compose12 (at level 45, right associativity).
  Infix "'.|'" := compose13 (at level 45, right associativity).
