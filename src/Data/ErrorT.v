...
Section error_t_Bijection.
  ...
  Global Instance IR_un_error_t_eq
    : InjectionRespect (error_t E m A) (m (error E A)) un_error_t eq eq.
  Proof.
  ...
  Global Instance II_ErrorT_eq
    : InjectionInverse (m (error E A)) (error_t E m A) ErrorT un_error_t eq.
  Proof.
    constructor ; auto.
  Qed.
...
Module error_t_DE_Arg <: DE_IdxTransformer_Arg.
  ...
  Definition to : forall {E m A}, T E m A -> U E m A := @un_error_t.
...
Section Proper.
  ...
  Qed.
  Global Instance un_error_t_Proper : Proper eqv (@un_error_t E m A).
  Proof.
    unfold Proper.
    logical_eqv_intro.
    destruct x,y ; simpl.
...
Hint Extern 9 (Proper eqv (ErrorT (E:=?E) (m:=?m) (A:=?A))) =>
  ...
  : typeclass_instances.
Hint Extern 9 (Proper eqv (un_error_t (E:=?E) (m:=?m) (A:=?A))) =>
  let H := fresh "H" in
  pose (H:=(un_error_t_Proper (E:=E) (m:=m) (A:=A))) ; apply H
  : typeclass_instances.
...
Section Monad.
  ...
  Section error_t_MonadWF.
    ...
    Proof.
      ...
      - simpl ; apply InjectionRespect_beta ; simpl.
        transitivity (un_error_t aM >>= funit).
        + logical_eqv.
