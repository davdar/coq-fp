Require Import Data.Comparison.
Require Import Data.Function.
Require Import Data.OptionStructures.
...
Section two3tree.
   ...
   *)
  | Two_t : two3tree -> K*V -> two3tree -> two3tree
  (*                       [a][b]
   * Three_t X a Y b Z =  /  |  \
   *                     X   Y   Z
   * Invariant: x in X => x < a; y in Y => a < y < b; z in Z => z > b
   *)
  | Three_t : two3tree -> K*V -> two3tree -> K*V -> two3tree -> two3tree
  .

  Fixpoint height_t (t:two3tree) : nat :=
  match t with
  | Null_t => O
   ...
   *                       ?   Y
   *)
  | TwoLeftHole_c : K*V -> two3tree -> context -> context
  (*                          C
   ...
   *                        X   ?
   *)
  | TwoRightHole_c : two3tree -> K*V -> context -> context
  (*                             C
   ...
   *                         ?   Y   Z
   *)
  | ThreeLeftHole_c : K*V -> two3tree -> K*V -> two3tree -> context -> context 
  (*                               C
   ...
   *                           X   ?   Z
   *)
  | ThreeMiddleHole_c : two3tree -> K*V -> K*V -> two3tree -> context -> context
  (*                                C
   ...
   *                   X   Y
   *)
  | TwoHole_l : two3tree -> two3tree -> context -> location
  (*                         C
   * TwoHole_l X Y b Z C =   |
   *                       [?][b]
   *                       /  |  \
   *                      X   Y   Z
   *)
  | ThreeLeftHole_l : two3tree -> two3tree -> K*V -> two3tree -> context -> location
  (*                         C
   * TwoHole_l X a Y Z C =   |
   *                       [a][?]
   *                       /  |  \
   *                      X   Y   Z
   *)
  | ThreeRightHole_l : two3tree -> K*V -> two3tree -> two3tree -> context -> location
  .

  Definition fillLocation (e:K*V) (l:location) : two3tree :=
  match l with
  ...
  Fixpoint locate (k:K) (t:two3tree) (c:context) : context + (K*V) * location :=
  match t with
  | Null_t => inl c
  | Two_t tl em tr =>
      match compare k (fst em) with
  ...
  | Three_t tl el tm er tr =>
      match compare k (fst el), compare k (fst er) with
  ...
  Fixpoint locateGreatest (t:two3tree) (c:context)
    : option ((K*V) * (context + (K*V) * context)) :=
  match t with
  | Null_t => None
  | Two_t tl em tr => liftM collapse $
      locateGreatest tr (TwoRightHole_c tl em c)
      <+>
      ret (em, inl c)
  | Three_t tl el tm er tr => liftM collapse $
   ...
   * its context.
   *)
  Fixpoint insertUp (tet:two3tree * (K*V) * two3tree) (c:context) : two3tree :=
  let '(tl,em,tr) := tet in
  match c with
  (*     _          
   *     |          
  ...
  Definition insert (e:K*V) (t:two3tree) : two3tree :=
  match locate (fst e) t Top_c with
   ...
   * context *)
  Fixpoint removeUp (t:two3tree) (c:context) : two3tree :=
  match c with
  (*  _        
   *  ||
   *  e  =>  e
  ...
  Definition remove (k:K) (t:two3tree) : two3tree :=
