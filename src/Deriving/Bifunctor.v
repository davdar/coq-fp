...
Class DerivingEverything_BifunctorI U :=
  { DE_Bif_EqDec :> Bif_EqDec U
  ; DE_Bif_Eq_RDC :> Bif_Eq_RDC U
  ; DE_Bif_Eqv :> Bif_Eqv U
  ; DE_Bif_PER_WF :> Bif_PER_WF U
  ; DE_Bif_ER_WF :> Bif_ER_WF U
  ; DE_Bif_EqvDec :> Bif_EqvDec U
  ; DE_Bif_Eqv_RDC :> Bif_Eqv_RDC U
  ; DE_Bif_Lte :> Bif_Lte U
  ; DE_Bif_PreOrd :> Bif_PreOrd U
  ; DE_Bif_LteDec :> Bif_LteDec U
  ; DE_Bif_Lte_RDC :> Bif_Lte_RDC U
  ; DE_Bif_PartialOrd :> Bif_PartialOrd U
  ; DE_Bif_PartialOrdDec :> Bif_PartialOrdDec U
  ; DE_Bif_PartialOrd_RDC :> Bif_PartialOrd_RDC U
  ; DE_Bif_TotalOrd :> Bif_TotalOrd U
  ; DE_Bif_TotalOrdDec :> Bif_TotalOrdDec U
  ; DE_Bif_TotalOrd_RDC :> Bif_TotalOrd_RDC U
  ; DE_Bif_Lattice :> Bif_Lattice U
  ; DE_Bif_LatticeWF :> Bif_LatticeWF U
  ; DE_Bif_BoundedLattice :> Bif_BoundedLattice U
  ; DE_Bif_BoundedLatticeWF :> Bif_BoundedLatticeWF U
  }.

Module Type DerivingEverything_Bifunctor_Arg.
  ...
  Parameter InjResp : forall {A B}, InjectionRespect (T A B) (U A B) to eq eq.
  Parameter InjInv : forall {A B}, InjectionInverse (U A B) (T A B) from to eq.
  Global Declare Instance _DerivingEverything_BifunctorI : DerivingEverything_BifunctorI U.
End DerivingEverything_Bifunctor_Arg.

Module DerivingEverything_Bifunctor (M:DerivingEverything_Bifunctor_Arg).
  Import M.
  Arguments T _ _ /.
  Arguments U _ _ /.
  Arguments to {A B} _ /.
  ...
  Section Eq.
    ...
    Local Instance _EqDec : EqDec (T A B) := { eq_dec := eq_dec `on` to }.
    Local Instance _Eq_RDC : Eq_RDC (T A B) := Deriving_Eq_RDC to eq_refl.
  End Eq.
  ...
  Section Eqv.
    ...
    Local Instance _Eqv : Eqv (T A B) := { eqv := eqv `on` to }.
    Global Instance _IR_to_eqv : InjectionRespect (T A B) (U A B) to eqv eqv :=
      Deriving_IR to eqv eqv eq_refl.
    Local Instance _EqvDec : EqvDec (T A B) := { eqv_dec := eqv_dec `on` to }.
    Local Instance _Eqv_RDC : Eqv_RDC (T A B) :=
  ...
  Section PER_WF.
    ...
    Local Instance _PER_WF : PER_WF (T A B) := Deriving_PER_WF to.
  ...
  Section ER_WF.
    ...
    Local Instance _ER_WF : ER_WF (T A B) := Deriving_ER_WF to.
  ...
  Section PreOrd.
    ...
    Local Instance _Lte : Lte (T A B) := { lte := lte `on` to }.
    Global Instance _IR_to_lte : InjectionRespect (T A B) (U A B) to lte lte :=
      Deriving_IR to lte lte eq_refl.
    Local Instance _PreOrd : PreOrd (T A B) := Deriving_PreOrd to.
    Local Instance _LteDec : LteDec (T A B) := { lte_dec := lte_dec `on` to }.
    Local Instance _Lte_RDC : Lte_RDC (T A B) := Deriving_Lte_RDC to eq_refl.
  End PreOrd.
  ...
  Section PartialOrd.
    ...
    Local Instance _PartialOrd : PartialOrd (T A B) := Deriving_PartialOrd to.
    Local Instance _PartialOrdDec : PartialOrdDec (T A B) := { pord_dec := pord_dec `on` to }.
    Local Instance _PartialOrd_RDC : PartialOrd_RDC (T A B) :=
  ...
  Section TotalOrd.
    ...
    Local Instance _TotalOrd : TotalOrd (T A B) := Deriving_TotalOrd to.
    Local Instance _TotalOrdDec : TotalOrdDec (T A B) := { tord_dec := tord_dec `on` to }.
    Local Instance _TotalOrd_RDC : TotalOrd_RDC (T A B) :=
  ...
  Section Lattice.
    ...
    Local Instance _Lattice : Lattice (T A B) :=
    ...
    Local Instance _LatticeWF : LatticeWF (T A B) := Deriving_LatticeWF to.

    Local Instance _BoundedLattice : BoundedLattice (T A B) :=
      { ltop := from ltop
      ; lbot := from lbot
      }.
    Local Instance _BoundedLatticeWF : BoundedLatticeWF (T A B) :=
