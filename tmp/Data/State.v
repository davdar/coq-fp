...
Section MonadTrans.
  ...
  Definition state_t_lift {m} {M:Monad m} {A} (aM:m A) : state_t S m A :=
    StateT $ fun s => bind_fmap (flip pair s) aM.
...
Section state_t.
  ...
  Definition run_state_t {A} : S -> state_t S m A -> m (A * S) :=
    flip un_state_t.
  Definition eval_state_t {A} : S -> state_t S m A -> m A :=
    bind_fmap fst '..' run_state_t.
  Definition exec_state_t {A} : S -> state_t S m A -> m S :=
    bind_fmap snd '..' run_state_t.

  Definition state_t_funit {A} (a:A) : state_t S m A := StateT $ fun s => ret (a,s).
  Global Instance state_t_FUnit : FUnit (state_t S m) :=
    { funit := @state_t_funit }.

  Definition state_t_bind {A B}
      (aM:state_t S m A) (f:A -> state_t S m B) : state_t S m B :=
    StateT $ fun s => begin
      as' <- un_state_t aM s ;;
      let (a,s') := as' in
      un_state_t (f a) s'
  ...
  Global Instance state_t_MonadState : MState S (state_t S m) :=
    { get := @state_t_get
    ; put := @state_t_put
    }.

  Section MonadError.
    Context {E} {ME:MError E m}.
    ...
    Global Instance state_t_MonadError : MError E (state_t S m) :=
      { throw := @state_t_throw
      ; catch := @state_t_catch
      }.
  End MonadError.
